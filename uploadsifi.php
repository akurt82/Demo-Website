...
if ( $uplForceHeader == "" ) { $uplMore = "http://localhost/uploadsifi.php"; }
// Der Aufruf zur Datei
if ( $uplForceHeader == "" ) { $uplPass = "uploadsifi.php?".session_id()."&sessno=".$_GET['sessno']."&pid=".$_GET['pid']."&"; }
...
$query=mysql_query("SELECT * FROM worker_products WHERE id = '".$_GET['pid']."'");
if(!$query) die("Fehler bei der Abfrage: ".mysql_error());
while ( $datensatz = mysql_fetch_array($query) ) {
	$bild1 = $datensatz['bild1'];
	$bild2 = $datensatz['bild2'];
	$bild3 = $datensatz['bild3'];
	$bild4 = $datensatz['bild4'];
	$bild5 = $datensatz['bild5'];
}

if ( $_GET['sessno'] == 1 ) { $_SESSION['worker.station.bild'.$_GET['sessno']] = $bild1; } else
if ( $_GET['sessno'] == 2 ) { $_SESSION['worker.station.bild'.$_GET['sessno']] = $bild2; } else
if ( $_GET['sessno'] == 3 ) { $_SESSION['worker.station.bild'.$_GET['sessno']] = $bild3; } else
if ( $_GET['sessno'] == 4 ) { $_SESSION['worker.station.bild'.$_GET['sessno']] = $bild4; } else
if ( $_GET['sessno'] == 5 ) { $_SESSION['worker.station.bild'.$_GET['sessno']] = $bild5; }

if ( ! $_SESSION['worker.station.bild'.$_GET['sessno']] ) {
	$uplRemoved = "imgs/none.png";
} else {
	$query=mysql_query("SELECT * FROM aj_images WHERE id = '".$_SESSION['worker.station.bild'.$_GET['sessno']]."'");
...
<script type = "text/javascript">
	...
	function uplSifiThatImage ( i, m, f ) {
		if ( ( document.getElementById("uplsifiImage") ) && ( document.getElementById("uplsifiLink") ) ) {
			...
			// *** //
			svr.Sess("worker.station.bild<?php echo $_GET['sessno'] ?>", i );
		} else {
